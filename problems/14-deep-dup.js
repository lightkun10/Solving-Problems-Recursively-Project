...
let deepDup = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (Array.isArray(ele)) {
      // If the element is an array, recursively call deepDup on that element
      let subArrDup = deepDup(ele);
      dupArr.push(subArrDup);
    } else {
      // Base case; If the element is not an array, add it to the new non dup array
      dupArr.push(ele);
    }
  }
